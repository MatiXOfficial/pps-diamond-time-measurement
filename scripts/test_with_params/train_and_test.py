...
class Parameters:
    ...
    def __str__(self) -> str:
        return f"{self.filter_mult}"

parameter_sets = [
    Parameters(filter_mult=1),
    Parameters(filter_mult=4)
]
...
OUTPUT_PATH = f'/scripts/test_with_params/output/{i_param}'
RESULTS_PATH = '/scripts/test_with_params/output/'
WEIGHTS_PATH = f'/scripts/model_weights/test_with_params/{i_param}'
...
history = train_model(model, y_train=Y_heatmap_train, y_test=Y_heatmap_test, lr=0.001, name='test_unet_model', train=False, n_epochs=2000, lr_patience=50, es_patience=200, loss_weights=100, verbose=2)
...
plt.close()
plot_history(history, "Test UNet Model", ymax=0.4, savefig=PWD + OUTPUT_PATH + '/test_unet_model_history.png')
...
with open(PWD + RESULTS_PATH + '/results.csv', 'a') as file:
    file.write(f"{i_param}, {parameter_set}, {std*1000:0.2f}\n")
